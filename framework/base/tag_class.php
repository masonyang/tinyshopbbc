		...
		if($matches[1]!=='/')
		{
			switch($matches[2].$matches[3])
			{
				...
				case 'query:':
				{
                    ...
                    if(!isset($attrs['issalve'])) $issalve = 'salve';
                    else $issalve = $attrs['issalve'];

                    if(!isset($attrs['domain'])){
                        $domain = null;
                    }else {
                        if('zd' == $attrs['domain']){
                            $domain = Config::getInstance('config')->get('headStore');
                        }else{
                            $domain = $attrs['domain'];
                        }
                    }

					$tem = $id.' = new Query("'.$attrs['name'].'","'.$domain.'","'.$issalve.'");';
					//实现属性中符号表达式的问题
					$old_char=array(' ne ',' eq ',' lt ',' gt ',' le ',' ge ');
					$new_char=array(' != ',' = ',' < ',' > ',' <= ',' >= ');
