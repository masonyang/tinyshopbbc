...
class Controller extends Object
{
    ...
    public function render($view,$data=null,$return=false)
    {
        ...
        $runfile= Tiny::app()->getRuntimePath().DIRECTORY_SEPARATOR.$serverConfig['menu'].DIRECTORY_SEPARATOR.$this->getId().DIRECTORY_SEPARATOR.$view.$this->scriptExt;
        ...
        if(is_file($viewfile))
        {
            ...
            if(!file_exists($runfile) || filemtime($runfile)<filemtime($viewfile) || ($layoutexists && filemtime($runfile)<filemtime($layoutfile)))
            {
                ...
                if($layoutexists)
                {
                    ...
                    if(isset($authority['layout']['file']) && (strpos($layoutfile,$authority['layout']['file']) !== false)){
                        $nodisplay = count($authority['layout']['nodisplay']);
                        $replace = array();
                        for($i=0;$i<$nodisplay;$i++){
                            $replace[] = '';
                        }
                        $theme_str = str_replace($authority['layout']['nodisplay'],$replace,$theme_str);
                    }
                    $tem = str_replace("{__viewcontents}",$tem,$theme_str);
                }

                if(isset($authority[$view])){
                    if(isset($authority[$view]['nodisplay'])){
                        $nodisplay = count($authority[$view]['nodisplay']);
                        $replace = array();
                        for($i=0;$i<$nodisplay;$i++){
                            $replace[] = '';
                        }
                        $tem = str_replace($authority[$view]['nodisplay'],$replace,$tem);
                    }

                    if(isset($authority[$view]['justshow'])){
                        $justshow = $authority[$view]['justshow'];
                        $tem = str_replace($justshow['origin'],$justshow['replace'],$tem);
                    }
