...
class Model
{
     ...
     * @param mixed $name
     */
    public function __construct($name=null,$domain = null,$issalve = 'salve')
    {

		if(null === $domain || empty($domain)){
			$serverName = Tiny::getServerName();
			$domain = $serverName['top'];
		}

		$dbinfo = DBFactory::getDbInfo($domain,$issalve);
		$this->md5key($name,$dbinfo['domain'],$issalve);
        $this->db[$this->md5key] = DBFactory::getInstance($domain,$issalve);
		$this->prefix = $dbinfo['tablePre'];
        $this->table($name);
    }

	protected function md5key($name = null,$domain = null,$issalve = 'salve')
    {
        if($issalve == 'salve'){
            $salve = 'salve';
        }else{
        ...
        $this->md5key = md5($name.'-'.$domain.'-'.$salve);
    }
    ...
    public function insert()
    {
        ...
        if(is_array($data))
        {
            ...
            {
                if(is_string($key) && isset($this->fields[$this->md5key][$key]))
                {

                    if(is_null($val)){
                        $value = 'NULL';
                    }else{
                        $value = $this->formatField($this->fields[$this->md5key][$key],$val);
                    }
                    if(is_scalar($value))
                    {
                        $fields .= '`'.$key.'`,';
    ...
    public function update()
    {
        ...
        {
            if(is_string($key) && $key != $this->primary_key && isset($this->fields[$this->md5key][$key]))
            {
                if(is_string($key))
                {
                    $value = $val;
    ...
    public function query($sql=null)
    {
        ...
        $this->initSql();
        return $this->db[$this->md5key]->doSql($sql);
    }
    /**
     * 表初始化信息
     *
    ...
    {
        if(empty($this->fields[$this->md5key]))
        {
            $cache = CacheFactory::getInstance();
            $info = $cache->get('table_'.$this->sql['table'].'_'.$this->md5key);
            if(!$info)
            {
