...
class AdminController extends Controller
{
    ...
    public function config()
	{
		...
		if('headstore' == $serverConfig['menu']){
			return $this->config_headstore();
		}else{
			return $this->config_branchstore();
		}

	}

	private function config_headstore()
	{
	...
	private function config_branchstore()
	{
		$group = Req::args('group');
		if(Req::args('submit')!=null)
		{
	...
	public function send_email_test()
	{
		...
		if(Validator::email($test_email)){
			$mail = new Mail();
			try{
				$flag = $mail->send_email($test_email,'109电子商务系统,邮箱验证测试',"<div><h3>亲爱的109用户您好:</h3><p style='text-indent:2em;'>当您收到这封邮件时，说明您的信息配制正确，邮件系统可以正常工作。</p></div>");
