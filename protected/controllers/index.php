...
class IndexController extends Controller{
    ...
    public function attention()
    {
        ...
        if(isset($this->user['name'])){
            ...
            $obj = $attentionModel->where("goods_id=$goods_id and user_id=".$this->user['id'])->find();
            if($obj) $info = array('status' => 2);
            else {
                $attentionModel->data(array('goods_id'=>$goods_id,'user_id'=>$this->user['id'],'time'=>date('Y-m-d H:i:s')))->insert();
                $info = array('status' => 1);
            }
        }
    ...
    public function goods_consult()
    {
        ...
        if(isset($this->user['id']) && $this->user['name']){
            if($code == $verifyCode){
                $model = new Model("ask");
                $model->data(array('question'=>$content,'user_id'=>$this->user['id'],'goods_id'=>$id,'ask_time'=>date('Y-m-d H:i:s')))->insert();
    ...
    public function product()
    {
        ...
        $goodsModel = new Model("goods as go");
        $goods = $goodsModel->join("left join tiny_goods_category as gc on go.category_id = gc.id")->fields("go.*,gc.path")->where("go.id=$id and go.is_online=0")->find();
        if($goods){
            ...
            $categoryGoodsModel = new Model("goods_category");
            $category = $categoryGoodsModel->where("id in ($path)")->order("field(`id`,$path)")->findAll();
            $time = "'".date('Y-m-d H:i:s')."'";

            $promModel = new Model("prom_goods");
            $prom = $promModel->where("id=".$goods['prom_id']." and is_close=0 and $time >=start_time and $time <= end_time")->find();


            $attr_array = unserialize($goods['attrs']);
            $goods_attrs = array();
            if($attr_array){
                $model = new Model("goods_attr as ga");
                $rows = $model->fields("ga.*,av.name as vname,av.id as vid")->join("left join attr_value as av on ga.id=av.attr_id")->where("ga.type_id = $goods[type_id]")->findAll();
                $attrs = $_attrs = array();
                foreach ($rows as $row) {
                    $attrs[$row['id'].'-'.$row['vid']] = $row;
                    $_attrs[$row['id']] = $row;
            ...
            $rows = $reviewModel->fields("count(id) as num,point")->where("status=1 and goods_id = $id")->group("point")->findAll();
            foreach ($rows as $row) {
            ...
            $sale_protection = $helpModel->where("title='售后保障'")->find();
            if($sale_protection){
    ...
    private function parseCondition()
    {
        ...
        if($action=='search'){
            ...
            $where = "name like '%$keyword%'";
                ...
                $rows = $model->fields("category_id,count(id) as num")->where($where)->group("category_id")->findAll();
                $category_ids = "";
                ...
                if($category_ids){
                    ...
                    if($this->getModule()->checkToken()){
                        ...
                        if($len>=2 && $len<=8){
                            ...
                            $obj = $tagsModel->where("name='$keyword'")->find();
                            if($obj){
                                $tagsModel->data(array('num'=>"`num`+1"))->where("id=".$obj['id'])->update();
                            }else{
                                $tagsModel->data(array('name'=>$keyword))->insert();
                            }
                        }
                    }
                    $categoryGoodsModel = new Model("goods_category");
                    $rows = $categoryGoodsModel->where("id in ($category_ids)")->findAll();
                    foreach ($rows as $row) {
                        $path = trim($row['path'],',');
                        $paths = explode(',', $path);
                        $root = 0;
            ...
            if($cid!=0){
                ...
                $categoryGModel = new Model("goods_category as gc ");
                $category = $categoryGModel->join("left join goods_type as gt on gc.type_id = gt.id")->where("gc.id=$cid")->find();
                if($category){
        ...
        //关于分类检索的处理
            ...
            if($cid==0){
                ...
                $categoryGModel = new Model("goods_category as gc ");
                $current_category = $categoryGModel->fields("gc.*,gt.name as gname,gt.attr,gt.spec,gc.seo_title,gc.seo_keywords,gc.seo_description")->join("left join goods_type as gt on gc.type_id = gt.id")->where("gc.id = $cid")->find();
                if($current_category){
                    $path  = trim($current_category['path'],',');
                    $categoryModel = new Model("goods_category");
                    $rows = $categoryModel->where("path like '$current_category[path]%'")->order("field(`id`,$path)")->findAll();
                    $category = $categoryModel->where("id in ($path)")->order("field(`id`,$path)")->findAll();
            ...
            $categoryModel = new Model("goods_category");
            $seo_category = $categoryModel->where("id=$cid")->find();
            if($seo_category){
        ...
        //品牌筛选
        $goodsModel = new Model("goods as go");
        $rows = $goodsModel->fields("brand_id,count(id) as num")->where($where)->group("brand_id")->findAll();
        $brand_ids = '';
        ...
        //价格区间
        $prices = $goodsModel->fields("max(sell_price) as max,min(sell_price) as min,avg(sell_price) as avg")->where($where)->find();
        $price_range = Common::priceRange($prices);

        if($brand_ids){
            $brandModel = new Model("brand");
            $has_brand = $brandModel->where("id in ($brand_ids)")->findAll();
        }
        ...
        switch ($sort) {
            case '1':
                $goods_model = $goodsModel->join("left join tiny_order_goods as og on go.id = og.goods_id")->fields("go.*,sum(og.goods_nums) as sell_num")->order("sell_num desc")->group("go.id");
                break;
            case '2':
                $goods_model = $goodsModel->join("left join tiny_review as re on go.id = re.goods_id")->fields("go.*,count(re.goods_id) as renum")->group("go.id")->order("renum desc");
                break;
            case '3':
                $goods_model = $goodsModel->order("sell_price desc");
                break;
            case '4':
                $goods_model = $goodsModel->order("sell_price");
                break;
            case '5':
                $goods_model = $goodsModel->order("id desc");
                break;
            default:
                $goods_model = $goodsModel->order("sort desc");
                break;
        }
        //var_dump($where);exit;
        //提取商品
