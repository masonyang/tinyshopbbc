...
class pay_alipaymobile extends PaymentPlugin{
    ...
    public function callback($callbackData,&$paymentId,&$money,&$message,&$orderNo)
    {
        ...
        if (!empty($callbackData["notify_id"])) {$responseTxt = $this->getResponse($callbackData["notify_id"],$classConfig['partner_id']);}
...
//    }
    function rsaVerify($data, $ali_public_key_path, $sign)  {
        ...
        $alipay_public_key = file_get_contents($ali_public_key_path);
        //以下为了初始化私钥，保证在您填写私钥时不管是带格式还是不带格式都可以通过验证。
